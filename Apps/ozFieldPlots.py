...
for field in fieldReqs:
    ...
    for ts in timeSamplingIntervals:
        ...
        for fn in sampFiles:
            ...
            xTitle = x.min()
            yTitle = y.max()
            plt.text(xTitle, yTitle, plotTitle, ha='left', va='bottom', 
            ...
            if jobConfig['DisplayRegionNameOnFR']:
                ...
                plt.text(xRegLabel, yRegLabel, regLabel, ha='left', va='top', 
                         fontsize=8)
            ...
            #plt.savefig(frFile, pad_inches=0.1, dpi=300)
...
"""
print myName,':: Total Run TIme = ',time.time() - totalStartTime,' s.'
sys.stdout.flush()
avgRFTime = sum(readFileTimes) / float(len(readFileTimes))
print myName,':: Total Read File Time = ',sum(readFileTimes),' s.'
sys.stdout.flush()
print myName,':: Number of Read File Operations = ',len(readFileTimes),' with time/op of ',avgRFTime,' s.'
sys.stdout.flush()
print myName,':: Total BaseMap TIme = ',sum(baseMapTimes),' s.'
sys.stdout.flush()
avgBMTime = sum(baseMapTimes) / float(len(baseMapTimes))
print myName,':: Number of BaseMap Operations = ',len(baseMapTimes),' with time/op of ',avgBMTime,' s.'
sys.stdout.flush()
print myName,':: Total shapeFile read onto basemap Time = ',sum(shapeFileTimes),' s.'
sys.stdout.flush()
avgShFTime = sum(shapeFileTimes) / float(len(shapeFileTimes))
print myName,':: Number of shapeFile Operations = ',len(shapeFileTimes),' with time/op of ',avgShFTime,' s.'
sys.stdout.flush()
print myName,':: Total np.meshgrid TIme = ',sum(meshGridTimes),' s.'
sys.stdout.flush()
avgMGTime = sum(meshGridTimes) / float(len(meshGridTimes))
print myName,':: Number of meshgrid Operations = ',len(meshGridTimes),' with time/op of ',avgMGTime,' s.'
sys.stdout.flush()
print myName,':: Total bmap.pcolormesh() Times = ',sum(pColorTimes),' s.'
avgPCTime = sum(pColorTimes) / float(len(pColorTimes))
print myName,':: Number of bmap.pcolormesh() Operations = ',len(pColorTimes),' with time/op of ',avgPCTime,' s.'

print myName,':: Total save thumbnail image file time = ',sum(saveThumbnailFileTimes),' s.'
sys.stdout.flush()
avgSTnFTime = sum(saveThumbnailFileTimes) / float(len(saveThumbnailFileTimes))
print myName,':: Number of saveFile Operations = ',len(saveThumbnailFileTimes),' with time/op of ',avgSTnFTime,' s.'
print myName,':: Total save full-res image file time = ',sum(saveFullResFileTimes),' s.'
sys.stdout.flush()
avgSFRFTime = sum(saveFullResFileTimes) / float(len(saveFullResFileTimes))
print myName,':: Number of saveFile Operations = ',len(saveFullResFileTimes),' with time/op of ',avgSFRFTime,' s.'
sys.stdout.flush()
